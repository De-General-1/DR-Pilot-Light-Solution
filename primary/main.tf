...
module "s3_bucket_primary" {
  ...
  dr_bucket_name = var.dr-app-bucket-name
  primary_bucket_name = var.primary-app-bucket-name
}

module "lambda_backup" {
  source = "../modules/lambda_backup"

  lambda_file  = "../scripts/backup_lambda/backup_lambda.zip"
  lambda_hash  = filebase64sha256("../scripts/backup_lambda/backup_lambda.zip")
  db_host      = module.rds_primary.db_hostname
  db_user      = var.db_username
  db_password  = var.db_password
  db_name      = var.db_name
  s3_bucket    = module.s3_bucket_primary.primary_bucket_name
}
