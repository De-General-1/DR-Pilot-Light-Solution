...
module "rds_dr" {
  ...
  source_db_arn           = null # data.terraform_remote_state.primary.outputs.db_instance_arn
...
module "launch_template_dr" {
  ...
  user_data_install_docker = base64encode(file("../scripts/dr_user_data.sh"))
...
module "monitoring_primary" {
  ...
  health_check_id = null  # data.terraform_remote_state.primary.outputs.health_check_id
  snsEmail = var.snsEmail
}
